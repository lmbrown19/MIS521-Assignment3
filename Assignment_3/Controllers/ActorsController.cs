...
namespace Assignment_3.Controllers
{
    public class ActorsController : Controller
    {
        ...
        public ActorsController(Assignment_3Context context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var sentiment = await GetSentimentDataAsync(actor.name ?? "");
        ...
        public async Task<SentimentResult> GetSentimentDataAsync(string name)
        {
            ...
            List<string> textToExamine = await SearchRedditAsync(name);
            double totalScore = 0;
        ...
        public async Task<string> HuggingFaceMethod(string name)
        {
            var data = await GetSentimentDataAsync(name);
...

{
...

    {
        ...
        string json = await client.GetStringAsync("https://api.pullpush.io/reddit/search/comment/?size=25&q=" + WebUtility.UrlEncode(searchQuery)); JsonDocument doc = JsonDocument.Parse(json);
...
private static string TruncateAtTextElementBoundary(string input, int maxLength)
{
    ...
    return input.Substring(0, maxLength);
        ...
        public async Task<IActionResult> Create([Bind("id,name,gender,age,IMDBlink,mediaLink")] Actor actor)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int? id, [Bind("id,name,gender,age,IMDBlink,mediaLink")] Actor actor)
        {
            if (id != actor.id)
            {
                return NotFound();
